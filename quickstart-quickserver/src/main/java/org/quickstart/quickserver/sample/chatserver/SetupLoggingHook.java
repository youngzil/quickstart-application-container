...
package org.quickstart.quickserver.sample.chatserver;
...
public class SetupLoggingHook implements InitServerHook {
	...
	public void handleInit(QuickServer quickserver) throws Exception {
		...
		try	{
			...
			txtLog = new FileHandler("log/ChatServer_QuickServer%u%g.txt", 
				1024*1024, 20, true);
			...
			txtLog = new FileHandler("log/ChatServer%u%g.txt", 
				1024*1024, 20, true);
			txtLog.setLevel(Level.FINEST);
			txtLog.setFormatter(new SimpleTextFormatter());
			logger.addHandler(txtLog);
